...
This project uses the **Trellis** framework (.NET 10). Trellis combines Railway-Oriented Programming (ROP) with Domain-Driven Design (DDD).
...
> **Rule severity:**
> - 🔴 **Hard constraint** — violating this causes bugs, runtime errors, or architectural violations. No exceptions.
> - 🟡 **Default pattern** — follow this unless there's a clear, specific reason not to.
> - 🟢 **Acceptable variation** — deviation is fine when the stated condition applies.
...
The template provides the complete project structure. 🔴 Do NOT modify or recreate build system files (`Directory.Build.props`, `global.json`, `build/test.props`). They are pre-configured.
...
🔴 Handlers must return `Result`-based outcomes and must not throw for business logic.
