...
- 🔴 **`Maybe<T>` properties** — declare as `partial`. The source generator and `MaybeConvention` handle everything automatically — no manual backing fields or EF configuration needed. See §12 in `trellis-api-reference.md`. The `_camelCase` backing field is emitted by the source generator during `dotnet build` (see **Implementation Order and Build Checkpoints** above). 🔴 **The `Trellis.EntityFrameworkCore.Generator` package must be referenced in the Domain project** (as an Analyzer, with `ReferenceOutputAssembly="false"`) — not just in the Acl project. The template pre-configures this in `Domain.csproj`. Do NOT remove it — without the generator in the project that declares entities, `partial Maybe<T>` backing fields will not be emitted and EF Core persistence will silently fail.
